...
错误（HTTP 400），不在脏数据上做任何对齐。

**同一行的多个问题、以及跨行的连锁问题都会在一次提交里全部报出**：

- 某行时间非法且某压力列为 `NaN`，会同时给出两条带行列号的错误；
- 时间递增按**相邻原始行**逐行检查，基准与该行是否被采纳无关。因此即使第 N 行因
  压力非法被整行拒绝，第 N+1 行相对第 N 行的时间回退仍会在同一单报出——检修员修好
  压力重新提交时不会再被一个此前隐藏的时间错误打回，无需多次修单；
- 前一行时间无法解析时，它不提供基准，后一行相对更早的最近一个可解析时间比较。
